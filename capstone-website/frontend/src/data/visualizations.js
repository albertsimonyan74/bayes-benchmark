...
export const VISUALIZATIONS = [
  ...
  {
    id: 'grouped_bar',
    title: 'Score by Tier & Task Type',
    subtitle: 'Mean ± SD, broken out by difficulty tier',
    description: 'Grouped bars show average scores per task type within each difficulty tier, with error bars (±1 SD). The horizontal line marks the 0.5 pass threshold. Helps pinpoint exactly which task types drag each tier\'s average down.',
    png: '/visualizations/png/08_grouped_bar.png',
    interactive: '/visualizations/interactive/08_grouped_bar.html',
    type: 'Bar Chart',
    category: 'Comparisons',
    insight: 'Tier 3 has the highest score variance — advanced Bayesian topics produce wildly inconsistent results, while basic Tier 1 tasks are either mostly right or mostly wrong.',
  },
  {
    id: 'latency_accuracy',
  ...
  },
  {
    ...
    subtitle: 'Which model makes which mistakes?',
    description: 'Heatmap of error counts per model per error type. Brighter = more failures of that type. Claude has the fewest total failures (19). Shows whether a model\'s failures cluster around one type or spread across many.',
    png: '/visualizations/png/16b_error_by_model_heatmap.png',
    interactive: null,
    type: 'Heatmap',
    category: 'Error Analysis',
    insight: 'Claude is the most reliable model (fewest failures overall). E7 Truncation is universal — every single model hits the 1024-token cap on complex multi-output tasks like REGRESSION. Gemini has disproportionately high E4 Format Failures.',
  },
  {
    id: 'error-by-task',
    title: 'Error Taxonomy: By Task Type',
    subtitle: 'Different tasks fail for different reasons',
    description: 'Stacked bars for the 12 task types with the most failures, showing what mix of error types causes each task type to fail. Some tasks fail because of token limits; others because models misunderstand the statistical concept.',
    png: '/visualizations/png/16c_error_by_task_type.png',
    interactive: null,
    type: 'Bar',
    category: 'Error Analysis',
    insight: 'REGRESSION and BAYES_REG fail almost entirely due to E7 Truncation (token limit). BOX_MULLER and FISHER_INFO fail due to E6 Conceptual Confusion — models misunderstand the transformation, not the math. Different problems, different fixes.',
  },
]
