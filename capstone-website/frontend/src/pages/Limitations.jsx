...
const CAVEATS = [
  {
    ...
    body: `Hallucination — fabricating priors, distributions, or data not grounded in the task
    prompt — was a candidate L1 failure category. The error taxonomy returned zero such
    classifications across all 143 audited failures. This is a real signal, not a missing case:
    every benchmark task has a closed-form or seeded ground truth, so models fail by skipping
    required assumptions or miscomputing — not by fabricating. Llama 3.3 70B may still
    under-surface hallucination on well-constrained tasks (E5/E8/E9 are flagged "use only if
    nothing else fits"), so the 0% rate is partly methodology artifact and partly a property of
    bounded numerical Bayesian problems.`,
  },
  {
  ...
  },
  {
    ...
    strictness spot-checks were run on borderline cases, but true cross-judge validation against
    an independent family (e.g., GPT-4 class or Claude Opus as judge) and multi-template
    ensembling are paper-scope future work. Cite Feuer et al. (2025) "Judgment Becomes Noise" for
    the systematic-noise framing; the 5-provider design is the architectural counter-design to
    family-preference bias.`,
  },
  {
    title: 'Scope-specific eligibility filters',
    body: `The keyword-judge disagreement headline (20.74%) is computed on a 2,850-eligible-runs
    subset of the full 3,220-run combined denominator. The 105 excluded base runs come from 21
    distinct task families — CONCEPTUAL / MINIMAX / BAYES_RISK plus the MARKOV_04 outlier — that
    share empty required_assumption_checks. Keyword and judge scoring of assumption articulation
    cannot be compared on tasks that don't require assumption articulation. The same eligibility
    filter applies to 2,100 of 2,365 perturbation runs, yielding 2,850 / 3,220 combined
    eligibility (88.5%). Self-consistency calibration is computed on a different scope: 161 of
    171 tasks. The 10 CONCEPTUAL tasks (CONCEPTUAL_01–10) ask the model to explain or justify
    rather than compute — they have no numerical answer against which to measure 3-rerun
    agreement. This is a methodological property of consistency-based confidence extraction, not
    a project shortcut. Token-level logprobs (Multi-Answer Confidence 2026) would be required to
    extend calibration analysis to free-form explanatory tasks. Both filters reflect the same
    principle: different methodological measurements require different task scopes, and headline
    numbers must declare their denominator transparently.`,
  },
  {
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        <FadeIn>
          ...
          <p style={{ color: 'rgba(232,244,248,0.7)', fontSize: 14, lineHeight: 1.75, maxWidth: 800, marginBottom: 28 }}>
            Seven caveats the methodology requires us to disclose. Each is a property of the design,
