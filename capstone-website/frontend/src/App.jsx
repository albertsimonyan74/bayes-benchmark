...
const RQS = [
  { id:'RQ1', tier:'PRIMARY', color:'#00FFE0',
    ...
    headline:[
      ...
      'Krippendorff α = 0.55 (assumption, n=1,095); negative α on reasoning quality (-0.099 [-0.152, -0.045]) and method structure (-0.042)',
      'Keyword-judge gap widens 3.7pp under perturbation',
    ],
    viewLink:'judge',
    detail:'Methodology contribution: keyword rubrics systematically overstate assumption-checking quality on Bayesian tasks. Across 3,195 eligible runs (1,095 base + 2,100 perturbation) the Llama 3.3 70B external judge (Together AI) flags 708 keyword-judge disagreements on assumption_compliance. Krippendorff α (base, n=1,095) = 0.55 on assumption; α = -0.099 on reasoning_quality (CI [-0.152, -0.045] excludes zero) and -0.042 on method_structure. Combined-scope α (n=3,195) reports assumption 0.605, reasoning -0.080, method -0.016. Spearman ρ = 0.59 on assumption.',
  ...
  { id:'RQ2', tier:'SUPPORTING', color:'#FFB347',
    ...
    viewLink:'tasks',
    detail:'Domain-specific failure surface that prior benchmarks cannot expose. REGRESSION cluster, MCMC, and ADVANCED tasks score below cohort mean across every model. Under literature-derived NMACR weights, Gemini ranks #1 on accuracy (0.7326 [0.7117, 0.7532]).',
  ...
  { id:'RQ4', tier:'SUPPORTING', color:'#4A90D9',
    ...
    headline:[
      'Three different #1s: Gemini (accuracy), Mistral (robustness — Δ=−0.004), ChatGPT (calibration)',
      'Three of five models are statistically noise-equivalent on robustness (Mistral, ChatGPT, Gemini)',
      'Gemini accuracy #1 but robustness #3 — perturbations no longer break it sharply',
    ],
    viewLink:'robustness',
    detail:'2,365 perturbation runs (5 models × 473 perturbations, 0 errors). Under literature-derived NMACR weights, Mistral ranks #1 on robustness with NEGATIVE delta (perturbations slightly improve its scores). Mistral, ChatGPT, and Gemini robustness CIs all cross zero (noise-equivalent); only Claude and DeepSeek separate from zero. Three uniformly-robust task types: HIERARCHICAL, RJMCMC, VB.',
